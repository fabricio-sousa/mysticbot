...
def get_financial_data():
    if not os.path.exists(FILE_PATH): return {"error": f"File not found at: {FILE_PATH}"}
    try:
        with open(FILE_PATH, 'r', encoding='utf-8') as f:
            data = json.load(f)
    except: return {"error": "Invalid JSON in trades.json"}
    
    df = pd.DataFrame(data)
    if df.empty: return {"error": "No trade data found."}
...
def index():
    ...
    if "error" in data: return f"<h2 style='color:white;'>{data['error']}</h2>"
    ...
    <head>
        ...
        <style>
            ...
            .rsi-container { height: 380px; overflow: hidden; }
            @media (max-width: 768px) { .column { min-width: 100%; } }
        </style>
        <meta http-equiv="refresh" content="60">
    </head>
    <body>
        <div class="header"><h1>Mystic Bot</h1><div style="color:var(--green); font-size:11px;">● LIVE & PROTECTED</div></div>
        
        <div class="active-banner">
            <div><div style="font-size:10px; color:#8b949e;">ACTIVE BLOCK</div><div class="banner-val">{{ window.label }}</div></div>
            <div><div style="font-size:10px; color:#8b949e;">RISK</div><div class="banner-val" style="color:{{ 'var(--green)' if window.risk != '1%' else 'var(--gold)' }}">{{ window.risk }}</div></div>
        </div>

        <div class="stats-grid">
            <div class="card"><span style="font-size:10px; color:#8b949e;">TOTAL PNL</span><span class="card-val {{ 'pos' if total_pnl >= 0 else 'neg' }}">${{ "%.2f"|format(total_pnl) }}</span></div>
            <div class="card"><span style="font-size:10px; color:#8b949e;">DAILY</span><span class="card-val {{ 'pos' if daily_pnl >= 0 else 'neg' }}">${{ "%.2f"|format(daily_pnl) }}</span></div>
            <div class="card"><span style="font-size:10px; color:#8b949e;">WIN RATE</span><span class="card-val pos">{{ "%.1f"|format(win_rate) }}%</span></div>
        </div>

        <div class="main-layout">
            <div class="column">
                <div class="section-title">Live Momentum (RSI)</div>
                <div class="panel rsi-container">
            ...
            <div class="column">
                ...
                <div class="panel table-container">
                    <table>
                        <thead><tr style="color:#8b949e;"><th>Time</th><th>PNL</th><th>Result</th></tr></thead>
                        <tbody>
                            {% for row in trades %}
    ...
    """
    return render_template_string(html_template, trades=trades_list, total_pnl=data['total_pnl'], daily_pnl=data['daily_pnl'], win_rate=data['win_rate'], window=current_win, schedule=STRATEGY_SCHEDULE)
