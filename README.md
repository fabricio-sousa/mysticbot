...
## 📊 Live Performance Dashboard (`dashboard.py`)

The repository now includes a Flask-based web dashboard that provides real-time visualization of the bot's activity.

* **Real-Time Stats:** Tracks Total PnL, Daily PnL, and Win Rate.
* **Visual Schedule:** Displays the current active trading block and risk level.
* **Trade History:** Lists recent trades directly from `trades.json`.
* **Remote Access:** Integrated with **pyngrok** to generate a secure public URL for monitoring the bot from any device.
* **Security Overview:** Displays active guardrails (Stop Loss, Hard Floor, etc.) to ensure the bot is operating within parameters.

## 🛠️ Tech Stack

* **Language:** Python 3.x
* **Framework:** Flask (Dashboard)
* **Exchange API:** Kalshi API v2 (Python SDK)
* **Tunneling:** pyngrok
* **Environment:** Optimized for Windows (includes `winsound` audio alerts and `msvcrt` keyboard interrupts).

## 📂 Folder Structure

To run this bot locally, you must manually create the following files (which are ignored by this repo for security):

- `apikey.txt`: Your Kalshi API Key ID.
- `private.txt`: Your Kalshi RSA Private Key.
- `trades.json`: Generated by the bot to store performance data.
